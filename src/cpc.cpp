...
// free
	...
	std::vector<std::string> request_urls(
		 ...
		 */
	{
		NodeDirectory directory = directory(node);

		std::vector<std::string> urls;

		for(Address addr : directory.addresses) {
	...
	ReqState detect(CloudCoin& coin, std::set<NodeNum> nodes, std::set<NodeID>* const bl)
	{	
		if(nodes.empty())
			generate_n(back_inserter(nodes), NODE_COUNT

		std::array<std::string, CPC_NODE_COUNT> pan;
		generate_n(begin(pan), CPC_NODE_COUNT, std::function<std::string()> {guid});
